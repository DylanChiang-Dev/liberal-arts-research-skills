...
確認：範本「引用」CONVENTIONS §8 而非重抄定義（守單一真相源）；示意列標「（示意）」避免被誤當真資料；表尾留一空列供填寫。
...
若使用者有維護工作痕跡表（見 CONVENTIONS.md §8），查核完成後按其格式，把每筆引用追加為一列（標記定義以 §8 為準，本步不重抄）。狀態映射如下，**嚴守「查無≠偽造」——本 skill 永遠不在痕跡表寫入 🗑 或「假」**：
...
若使用者有維護工作痕跡表（見 CONVENTIONS.md §8），把本次審查產生的內容按其格式追加為列（標記定義以 §8 為準）：
...
```markdown
- 若使用者維護工作痕跡表，按 CONVENTIONS §8 追加列：✅已核實→✅已查證、⚠️/❌/❓→❓待查（附依據或更正建議）。
```
...
Read `MEMORY.md`，在合適區段加一行指向本功能（一句話 hook：工作痕跡表＝跨三個誠信 skill 的統一留痕格式，定義在 CONVENTIONS §8）。
...
- **spec §設計②（放哪、叫什麼）** → Task 1（CONVENTIONS §8）、Task 2（templates/work-trace-table.md）。✅
