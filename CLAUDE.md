...
| `RULES.md` | 誠信鐵律（4 條硬規） |
| `ROUTER.md` | 技能路由表：agent 接任務先查此表定位 skill；不確定先喚 `research-roadmap` |
...
本庫維持「人文社科研究者可讀、可手動介入」的定位，**刻意不採用**（`VERIFICATION.md` 末節）：`_shared/` 大型共用 fragments、`manifest.yaml` 分片載入、多 agent 長跑 orchestrator、無人值守 pipeline、理工論文專用圖表／投稿包流程。`research-roadmap` 是「引導式精靈」會接力喚起下一個 skill，但每到「只有你能決定」的關卡硬停等使用者拍板——這與被禁止的無人值守 pipeline 的唯一差別就是「會不會停」。新增功能前先確認沒踩這條線。
